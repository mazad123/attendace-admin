...
function ProjectUpdatedDetails() {
  ...
  const getProjectUpdatedAllMessages = async () => {
    try {
      const res = await httpClient.get(
        `${PROJECT.GET_PROJECT_UPDATED_DETAILS.replace('{projectId}', !params ? projectId : params.toString())}?subjectId=${subjectId}&type=${'admin'}`
      );
      if (res.status === 200) {
  ...
  const getProjectDetail = async () => {
    try {
      // return await httpClient.get(PROJECT.GET_PROJECT_BY_ID.replace("{projectId}", projectId));
  ...
  // for showing already selected users and userList
  ...
  return (
    <>
      <div className="main_content_panel main_content_panel2 project-upadts-con">
            ...
            <div className="dashboard_card">
              <div className="projects-update-wrapper">
                ...
                {!showEditMessage.open ?
                  <CkEditor values={values} setValues={setValues} inputFields={inputFields} inputFields1={[]} userId={userId} /> : ""
                }
                <div>
                  <hr></hr>
                  <div className="head-title-wrap">
                        ...
                        <div className="main_follow_info d-flex">
                          ...
                          <div className="follow-up-info_name">
                            <div className="d-flex align-items-center gap-2">
                              {<img src={data?.user_id?.profile_image ? data?.user_id?.profile_image : BlankImage} alt="profile_image" />} {data?.user_id?.name}
                            </div>
                          </div>
                                  ...
                                  <button
                                    ...
                                    onClick={(e) =>
                                      setShowEditMessage({
                                        open: true,
                                        data: data,
                                        allSelectedRecipients: inputFields,
                                      })
                                    }
        ...
        {showEditMessage.open && <EditProjectMessageModal show={showEditMessage} onHide={handleClose} />}
