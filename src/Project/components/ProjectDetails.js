...
function ProjectDetails() {
  ...
  const [projectUpdateDetailConstantData, setProjectUpdateDetailsConstantData] = useState([]);
  const [archiveProjectModal, setArchiveProjectModal] = useState({ open: false, projectId: "" });
  ...
  const [displayText, setDisplayText] = useState('Latest 10 Updates');
  ...
  const getProjectUpdateDetail = async (pageNumber, isConstant = false) => {
    try {
      const res = await httpClient.get(
        `${PROJECT.GET_PROJECT_SUBJECTS_BY_ID}?projectId=${projectId}&type=${'admin'}&from=${dates.from}&to=${dates.to}&page=${pageNumber}`
      );
      if (res.status === 200) {
        if (isConstant) {
          setProjectUpdateDetailsConstantData(res.data.result.subjects);
        } else {
  ...
  // const getProjectUpdateDetailConstantData = async (pageNumber) => {
  //   try {
  //     const res = await httpClient.get(
  //       `${PROJECT.GET_PROJECT_SUBJECTS_BY_ID}?projectId=${projectId}&type=${'admin'}&from=${dates.from}&to=${dates.to}&page=${pageNumber}`
  //     );
  ...
  const handleRecentProjectUpdatesClick = () => {
    setCurrentPage(1);
    getProjectUpdateDetail(1);
    setDisplayText('Lastest 10 Updates');
  };

  const handleCloseOfArchiveProjectModal = () => {
    setArchiveProjectModal({ open: false, projectId: "" });
  };
  ...
  return (
    <>
      <div className="main_content_panel container">
              ...
              <div className="projects-update-wrapper">
                <div className="row">
                  ...
                  <div className="col-md-4 text-end">
                    <button title="Archive Project" type="button" className="shadow-none border-0 btn-success btn-sm"
                      onClick={(e) =>
                ...
                <div className="last-project-updates-wrap pt-4">
                  ...
                  {projectUpdateDetailConstantData.map((data, i) => (
        ...
        {archiveProjectModal.open && <ArchiveProjectModal show={archiveProjectModal.open} onHide={handleCloseOfArchiveProjectModal} projectId={archiveProjectModal.projectId} />}
      </div>
    </>
  );
}
