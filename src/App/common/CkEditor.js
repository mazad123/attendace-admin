...
function CkEditor({ values, setValues, fieldName, setMessage, type, inputFields = [], inputFields1 = [], userId = [] }) {
  ...
  //Workig with typing disable link
  // const handlePasteAndExitHyperlink = (editor) => {
  //   if (!editor || !editor.commands) return;

  //   // Step 1: Disable CKEditor's automatic link conversion when typing
  //   editor.plugins.get("AutoLink")?.forceDisabled("disableAutoLink");
  //   editor.commands.get("link")?.forceDisabled("disableAutoLink");

  //   // Step 2: Remove links automatically created when typing
  //   editor.model.document.on("change:data", () => {
  //     editor.model.change((writer) => {
  //       const root = editor.model.document.getRoot();
  //       for (const child of root.getChildren()) {
  //         if (child.is("element", "a")) {
  //           // Remove hyperlink attributes if added automatically
  //           writer.removeAttribute("linkHref", child);
  //         }
  //       }
  //     });
  //   });

  //   // Step 3: Allow pasting URLs and convert them into clickable links
  //   editor.editing.view.document.on('clipboardInput', (event, data) => {
  //     let pastedContent = data.dataTransfer.getData('text/plain');
  ...
  return (
    <div style={{ position: 'relative' }} className='text-editor'>
      <CKEditor
        ...
        // data={customDecode(values[fieldName] || '')}
        data={customDecode(values[field] || '')}
        onReady={(editor) => {
      ...
      {showDropdown && type !== "edit_candidate" && (
        <div ref={dropdownRef} style={{ position: 'absolute', top: '50px', left: '10px', width: '310px', border: '1px solid #ccc', background: '#fff', zIndex: "10", borderRadius: "5px", fontSize: "14px", boxShadow: "0px 0px 5px #e3e1e1", }} className="message-dropdown">
