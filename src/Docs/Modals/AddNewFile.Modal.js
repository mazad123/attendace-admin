...
function AddNewFile({ show, onHide, data, updateBreadCrumArray, callback }) {
  ...
  const fileTypes = ['JPG', 'PNG', 'GIF', 'TXT', 'PDF', '.XLS', 'XLSX', 'DOCX', 'ZIP', 'RAR'];
  ...
  const handleUsers = (event, index) => {
    ...
    setSelectedUser((selectedUser) => [...selectedUser, {user_id:id.value, permission}]);
  };
...
// old changes       
      // if(data.permittedUsers.length > 0){
      //   fileDetails["permittedUsers"] = [];
			// 	data.permittedUsers.map((user)=>{
...
// New Changes
      if(data.permittedUsers.length > 0){
        let parentSelectedUser = [];
				data.permittedUsers.map((user)=>{
          parentSelectedUser.push({ user_id: user.user_id.id, permission: user.permission });
				})
        // Update selectedUser after the submission is successful
        const updatedSelectedUser = [...selectedUser, ...parentSelectedUser];
        fileDetails["permittedUsers"] = updatedSelectedUser;
      }else{
        fileDetails["permittedUsers"] = selectedUser;
			}
    }else{
   ...
   const handlePermission = (newPermission) => {
      setPermission(newPermission);
      // Update the permission for all selected users when setPermission is called
      const updatedSelectedUser = selectedUser.map((user) => ({
        ...user,
