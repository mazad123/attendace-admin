...
function DeleteFileOrFolderModal(props) {
  const { show, onHide, type, treeViewData, nodeData, targetedData, callback, updateBreadCrumArray, from} = props;
  ...
  const getTargettedNodeChildrensIds =  async (data) => {
    if(data.children && data.children.length > 0){
      ...
      for(let currenElement=0; currenElement<data.children.length; currenElement++){
        if(data.children[currenElement].children && data.children[currenElement].children.length > 0){
...
if(targetedData){
  ...
  const deleteFileOrFolderModal = async () => {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        onHide();
        if(from==="file-explorer" || nodeData._id!==targetedData._id){
          callback(nodeData._id)
        }
        else{
          callback(nodeData.parentFolder)
        }
      }
    } catch (err) {
      console.log({err})
  ...
  return (
    <>
      ...
      {folderTreeView && <DirectoriesTree treeViewData={folderTreeView} />}
